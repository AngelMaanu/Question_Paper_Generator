...
def create_table():
    ...
    cursor=conn.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS reg1 (name text, email text, pass int,mark int)")
    conn.commit()
    conn.close()
create_table()
...
def register():
        ...
        conn=sqlite3.connect(db)
        cursor=conn.cursor()
        cursor.execute("insert into reg1 (name,email,pass,mark)values(?,?,?,?)",(name,email,password,mark))
        conn.commit()
        return render_template("index1.html")
...
def login():
     ...
     conn=sqlite3.connect(db)
     cursor=conn.cursor()
     cursor.execute("select name , pass  from reg1 WHERE email = ? AND pass = ?", (email, password))
...
# Add separate indices for python, java, and c questions
current_question_index_python = 0
current_question_index_java = 0
current_question_index_c = 0

@app.route('/generate_question')
def generate_question():
    global current_question_index_python
    data = index1()
    return process_question(data, current_question_index_python)

@app.route('/generate_question1')
def generate_question1():
    global current_question_index_java
    data = index1()
    return process_question(data, current_question_index_java)

@app.route('/generate_question2')
def generate_question2():
    global current_question_index_c
    data = index1()
    return process_question(data, current_question_index_c)

def process_question(data, index):
    if index < len(data):
        current_question = data[index]['question']
        question_number = data[index]['question_number']
        # Increment the index for the respective question type
        if l[-1] == "python":
            global current_question_index_python
            current_question_index_python += 1
        elif l[-1] == "java":
            global current_question_index_java
            current_question_index_java += 1
        elif l[-1] == "c":
            global current_question_index_c
            current_question_index_c += 1
...
@app.route('/get_question/<question_number>', methods=['GET'])
def get_question(question_number):
    ...
    question_number = int(question_number)
    if 0 < question_number <= len(data):
        question_number -= 1
        question = data[question_number]['question']
        return jsonify({'question_number': question_number + 1, 'question': question})
    else:
        return jsonify({'question_number': None, 'question': 'No more questions'})
...
def admin():
            conn=sqlite3.connect(db)
            cursor=conn.cursor()
            cursor.execute("SELECT name, email, mark FROM reg1")
...
def result2():
            ...
            cursor=conn.cursor()
            name=l1[-1]
            cursor.execute("SELECT name, email, mark FROM reg1 where name=?",(name,))
            data=cursor.fetchall()
            return render_template('admin.html', data = data)                
...
def forget():
    if request.method == 'POST':
        email = request.form['email']
        password = request.form['Password']
        password1 = request.form['nPassword']
        if password != password1:
            return "password mismatch"
        conn = sqlite3.connect(db)
        cursor = conn.cursor()
        cursor.execute("SELECT pass FROM reg1 WHERE email=?", (email,))
        data = cursor.fetchone()
        if data:
            cursor.execute("UPDATE reg1 SET pass=? WHERE email=?", (password1, email))
            conn.commit()
